...
function valid(value:unknown):value is Line{if(!value||typeof value!=='object')return false;const l=value as Line,p=products.find(p=>p.slug===l.slug);return !!p&&!p.accessory&&!!p.variants[l.variant]&&Number.isInteger(l.variant)&&Number.isInteger(l.quantity)&&l.quantity>0&&l.quantity<100&&l.plan==='once'&&(storeMode!=='rt'||l.slug==='inno-3-rt')&&(storeMode!=='get'||l.quantity===2||l.quantity===3)}
export function FocusedShell({children}:{children:ReactNode}){
 ...
 useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(storageKey)||'{}');if(Array.isArray(saved.lines)){const ls=saved.lines.filter(valid);if(storeMode!=='box'||ls.reduce((s:number,l:Line)=>s+l.quantity,0)<=5)setLines(storeMode==='box'?ls:ls.slice(0,1));}setBac(saved.bac===true)}catch{}setLoaded(true);fetch('/api/catalog',{cache:'no-store'}).then(r=>r.json() as Promise<{products?:Product[]}>).then(r=>{if(Array.isArray(r.products)&&r.products.length===products.length)setCatalog(r.products)}).catch(()=>{})},[]);
 ...
 <div className="announcement"><span>{storeMode==='box'?'Your box. Your picks. Up to 40% off.':storeMode==='get'?'Buy 2. Get your 3rd vial FREE.':'INNO-3 RT · Buy more. Save up to 30%.'}</span><a href="/#purchase">{storeMode==='box'?'Build your box':'Choose your offer'} <ArrowRight size={14}/></a></div>

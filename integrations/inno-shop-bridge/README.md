...
Each storefront sends its server-selected `offer` inside the signed handoff payload. `shop` retains subscriptions and its original quantity tiers. `box` accepts 1–5 one-time peptide vials at 15/20/30/35/40% and includes free BAC with 3+. `get` accepts any mix of peptide products and strengths totaling exactly 2 or 3 vials. The lowest-priced physical vial is half-price at quantity 2, or free at quantity 3. Duplicate selections merge; one discount is allocated across the whole offer. Half-price charges round to cents; line totals remain exact, including grouped quantities. BAC is excluded and other discounts do not stack. `rt` displays GLP-3 while preserving INNO-3 RT source product IDs. It accepts one size, 1–99 vials, and either one-time (15/20/30%) or monthly (25/30/40%) purchase. RT orders include one free BAC gift with 3+ vials; qualifying monthly shipments must each include that gift. RT quantities 1–2 can add one-time BAC for $10; this paid add-on must not renew. `get` offers optional BAC for $10 at all quantities.
...
## GLP-3 monthly adapter requirements

The adapter receives the offer profile as its fifth filter argument so it can distinguish RT monthly rates (25/30/40%) from shop monthly rates (20/25/35%). Validate the correct recurring product total, the selected quantity every month, one zero-priced BAC gift in each qualifying 3+ vial monthly shipment, and gift removal when quantity drops below three. Paid $10 BAC on a smaller initial order is one-time only. The generic bridge skips recurring carts and renewal carts; metadata alone does not implement renewal gifts. Keep the subscription guard in place until the actual WooCommerce extension and NMI recurring flow are integrated and tested.
